...
settings={ 'mpi_runner':'aprun',
           'physical_cores_per_node':24,
           'logical_cpus_per_core':2,
           'hyperthreading_allowed':True }

par=crow.sysenv.get_parallelism('AprunCrayMPI',settings)
sch=crow.sysenv.get_scheduler('LSFAlps',settings)
...
print(str(cmd))
print(str(res))
assert(str(cmd)=="ShellCommand(command=['aprun', '-n', '12', '/usr/bin/env', 'OMP_NUM_THREADS=4', 'exe1', ':', '-envall', '-n', '48', '/usr/bin/env', 'OMP_NUM_THREADS=1', 'exe2', ':', '-n', '200', 'exe2'], env=None, cwd=None, files=[ ])")
...
print(str(cmd))
print(str(res))
assert(str(cmd)=="ShellCommand(command=['mpiexec', '-n', '12', 'doit', ':', '-n', '12', 'doit'], env=None, cwd=None, files=[ ])")
